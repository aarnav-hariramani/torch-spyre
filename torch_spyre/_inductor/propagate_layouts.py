...
def _eager_view_input_layout(
    ...
    ptl: FixedLayout,
    name: str,
    stl: "SpyreTensorLayout | None" = None,
) -> "FixedLayout | None":
    ...
    ``stl`` is the SpyreTensorLayout to validate the offset against.  Callers
    pass the layout they already resolved for this input; it defaults to
    ``real_input.device_tensor_layout()``, which is what a forward graph input
    resolves to anyway.  A BACKWARD graph input must supply it: those
    placeholders are FakeTensors whose ``device_tensor_layout()`` is None by
    design, yet AOTAutograd still preserves a view-derived primal's
    ``storage_offset`` on the placeholder, so the rewrite is needed there too.
    ...
    if stl is None:
        # Nothing to validate the offset against, so leave the layout alone.
        # Unreachable from the forward path (the caller already skips an input
        # whose device_tensor_layout() is None); a backward caller always passes
        # the STL it resolved.
        return None
...
# Sentinel: this graph input gets no SpyreTensorLayout, so leave its FixedLayout
# and .layouts untouched.  Either it has no InputBuffer (SymInt / BackwardState),
# or it is a host-resident tensor with no Spyre layout (see
# _resolve_forward_input_stl).
_SKIP_INPUT = object()
...
def _resolve_forward_input_stl(name: str, real_input):
    ...
    Returns None for a non-tensor input (SymInt / BackwardState), or ``_SKIP_INPUT``
    for a tensor that has no Spyre layout at all.
    """
    if isinstance(real_input, torch.Tensor) and hasattr(
        ...
        stl = real_input.device_tensor_layout()
        if stl is None:
            # A CPU tensor lifted as a graph input, or a host tensor feeding a
            # FallbackKernel has no Spyre layout; leave its FixedLayout and
            # .layouts untouched, mirroring the non-Spyre ComputedBuffer skip in
...
def _resolve_graph_input_stl(name, real_input, graph, fwd_by_name):
    ...
    Returns the STL, or ``_SKIP_INPUT`` for a non-tensor input and for a
    host-resident tensor with no Spyre layout.  Raises for an unmatched non-tangent
    backward input (forward served from a compile cache).
    """
    if graph.is_backward:
...
def _assign_graph_input_layouts(graph: GraphLowering) -> None:
    ...
    for name, real_input in zip(graph.graph_input_names, inputs_to_zip):
        ...
        # Rewrite a placeholder view's FixedLayout to "layout = base, dep =
        # view".  Needed on the forward path and for a backward SAVED
        # ACTIVATION: AOTAutograd keeps a view-derived primal's storage_offset
        # on the backward placeholder too, and skipping the rewrite there would
        # leave the backward reading that saved activation from offset 0.
        #
        # Deliberately NOT done for a backward tangent.  A tangent placeholder
        # inherits the *forward output view's* storage_offset, but the tangent
        # actually passed at .backward() time is a freshly allocated gradient at
        # offset 0 (measured: traced 64 / 256 vs runtime 0), so that offset
        # describes no real buffer and must not be baked into the read.
        #
